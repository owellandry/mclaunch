...
TEMP_TAR="/tmp/temurin-21-linux-x64.tar.gz"
...
if [ "$CURRENT" -gt 0 ] 2>/dev/null; then
    ...
    log "missing" "Java no encontrado. Descargando Java 21 LTS (Temurin x64)..."
fi

# --- Intentar instalar via gestor de paquetes primero ---
install_via_pkg_manager() {
    if command -v apt &>/dev/null; then
        ...
        log "installing" "Instalando via yum..."
        sudo yum install -y java-21-openjdk-devel 2>/dev/null || return 1
        return 0
...
# --- Fallback: descarga manual de Temurin ---
log "downloading" "Descargando Eclipse Temurin 21 para Linux x64..."
if command -v curl &>/dev/null; then
    curl -fsSL "$JAVA_URL" -o "$TEMP_TAR"
elif command -v wget &>/dev/null; then
    wget -q "$JAVA_URL" -O "$TEMP_TAR"
else
    log "error" "curl ni wget estan disponibles. Instala uno de ellos e intenta de nuevo."
    exit 1
fi
