#!/usr/bin/env bash
# 02-check-java.sh
# Verifica si Java >= 21 esta instalado. Si no, descarga e instala Temurin 21 (macOS x64).
# Output: JSON con el resultado de cada paso

set -euo pipefail

MIN_JAVA=21
JAVA_URL="https://api.adoptium.net/v3/binary/latest/21/ga/mac/x64/jdk/hotspot/normal/eclipse"
TEMP_PKG="/tmp/temurin-21-macos-x64.pkg"

log() { printf '{"status":"%s","message":"%s"}\n' "$1" "$2"; }

get_java_major() {
    # macOS puede tener java via JAVA_HOME o /usr/libexec/java_home
    local java_bin="java"
...
if [ "$CURRENT" -gt 0 ] 2>/dev/null; then
    ...
    log "missing" "Java no encontrado. Instalando Java 21 LTS (Temurin x64)..."
fi

# --- Intentar via Homebrew ---
if command -v brew &>/dev/null; then
    log "installing" "Instalando via Homebrew..."
...
# --- Fallback: descarga .pkg ---
log "downloading" "Descargando Eclipse Temurin 21 .pkg para macOS x64..."
