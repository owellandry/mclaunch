...
import { useTextureInfo, type TextureInfo } from "@/presentation/lib/useTextureInfo";
...
const DEFAULT_ROTATION_Y = -32;
const ROTATION_X_MIN = -55;
...
export const MinecraftSkinFigure = memo(function MinecraftSkinFigure({
  ...
  const textureInfo = useTextureInfo(deferredTextureUrl, 64, 64);
  const capeTextureInfo = useTextureInfo(deferredCapeUrl, 64, 32);

  const [rotation, setRotation] = useState({ x: DEFAULT_ROTATION_X, y: DEFAULT_ROTATION_Y });
  ...
  const applyTransform = (rot: { x: number; y: number }) => {
    if (rotationGroupRef.current) {
      rotationGroupRef.current.style.transform =
        `translate3d(calc(${FIGURE_OFFSET_X}), ${FIGURE_OFFSET_Y}, 0px) rotateX(${rot.x}deg) rotateY(${rot.y}deg)`;
    }
  };
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const nextRotation = {
        x: Math.max(ROTATION_X_MIN, Math.min(ROTATION_X_MAX, rotationRef.current.x - deltaY * DRAG_SENSITIVITY_Y)),
        y: rotationRef.current.y + deltaX * DRAG_SENSITIVITY_X,
      };
  ...
  const resetView = () => {
    const nextRotation = { x: DEFAULT_ROTATION_X, y: DEFAULT_ROTATION_Y };
    rotationRef.current = nextRotation;
    ...
    >
      <div
        ...
        style={{
          ...
          transform: `translate3d(calc(${FIGURE_OFFSET_X}), ${FIGURE_OFFSET_Y}, 0px) rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)`,
        }}
      >
        <div
          className="[transform-style:preserve-3d]"
